...
function cerrarDialogoReserva(){
	$('#reserva_cliente_modal').modal('hide');
}

function cerrarDialogoMiReserva(){
	$('#reserva_propia_modal').modal('hide');
}
...
function activateEditCoach() {
	$('#lapizcoach').hide();
	$('#coach').hide();
	$('#coachinput').css('display','inline');
	$('#disquetecoach').show();
}
...
function saveNameActivity() {
	...
	$.getJSON(api_url+'activities/edit_name?callback=?', {id:id, name:name}, function(data){
		if(data.status=='success'){
...
function saveCoach() {
	...
	var name = $('#coachinput').val();
	$.getJSON(api_url+'schedules/edit_coach?callback=?', {id:id, name:name}, function(data){
		if(data.status=='success'){
			launch_alert('<i class="fa fa-smile-o"></i> Coach guardado','');
			$('#lapizcoach').show();
			$('#coach').html($('#coachinput').val());
			$('#coach').show();
			$('#coachinput').hide();
			$('#disquetecoach').hide();
			loadCalendar();
...
function mostrarDialogoInfoCompleta(cad,cad2,desc,aforo,minimo,disponibles,totales,diasantelacion,minutostope,consumocad,cancelacionminutos){
	...
	var cadcancelacionminutos = '';
	if(cancelacionminutos >=100){
		var cadnnn = cad2.split('-')[0];
		var horannn = parseInt(cadnnn.split(':')[0]);
	...
	$('#reserva_info_modal .modal-dialog').attr('style','width: 330px');
	$('#reserva_info_modal .modal-content').attr('border-radius','12px !important');
	$('#reserva_info_modal .modal-footer').hide();
	$('#reserva_info_modal .modal-title').attr('style','text-transform: capitalize; color: black; text-align: center; font-weight: 600; font-size: 15px; margin-top: 8px;')
	modalAddTitle(mymodal,cad);
	$('#reserva_info_modal .modal-body').html('<div style="text-align: center; margin-top: -16px; color: black; font-size: 13px; font-weight: 500;">Con horario '+cad2+'</div><div style="text-align: center;font-size: 11px;color: black;margin-top: 8px;">'+desc+'</div><div style="text-align: center;font-size: 11px;margin-top: 3px;color: black;">Aforo máximo de: '+aforo+' personas</div><div style="text-align: center;font-size: 11px;color: black;">Reservas mínimas para que haya clase: '+minimo+'</div><div style="text-align: center;font-size: 11px;color: black;">Plazas disponibles: '+disponibles+' / Plazas totales: '+totales+'</div><div style="font-size: 22px;color: red;text-align: center !important;font-weight: 700;text-align: left;margin-top: 8px;"><i class="fa fa-frown-o"></i></div><div style="font-size: 11px;color: red;text-align: left;margin-top: 3px;">AFORO DE ACTIVIDAD ACTUALMENTE COMPLETO</div><div style="font-size: 11px;color: #8064a2;text-align: center !important;text-align: left;margin-top: 3px;">ESPERAR EN COLA</div><div style="font-size: 11px;color: black;text-align: center;margin-top: 8px;">Actualmente NO hay plazas disponibles y NO es posible reservar la actividad.</div><div style="font-size: 11px;color: black;text-align: left;margin-top: 8px;">Puedes colocarte en la cola para la actividad. Si un usuario cancela su reserva de actividad antes del cierre de cancelación, que es como máximo '+cadcancelacionminutos+', el sistema te asignará la plaza automáticamente si estás el primero de la cola.</div><div style="font-size: 11px;color: black;text-align: left;margin-top: 8px;">Si esto sucede, te lo notificaremos vía e-mail y/o mensaje al móvil, dependiendo de como lo hayas configurado en tu perfil personal.</div><div style="font-size: 11px;color: black;text-align: left;margin-top: 8px;">Las reservas de esta actividad y tramo horario pueden hacerse con un máximo de '+diasantelacion+' días y un mínimo de '+cadminutostope+' de antelación a su hora de comienzo.</div><div style="text-align: center;text-transform: uppercase;background-color: #4bacc6;color: white;font-weight: 700;font-size: 12px;margin-top: 10px;cursor: pointer;margin-bottom: 10px;">ir a <span onclick="modal_eula(\'condiciones_uso\');">política de privacidad</span> y <span onclick="modal_eula(\'informacion_legal\');">aviso legal</span></div>');
}

function mostrarDialogoInfo(cad,cad2,desc,aforo,minimo,disponibles,totales,diasantelacion,minutostope,consumocad,cancelacionminutos){
	var mymodal=newModal('reserva_info_modal',true, true);
	modalAddTitle(mymodal,'');
	doModalBigger(mymodal);
	modalAddBody(mymodal,'<div class="waiting"><i class="fa fa-cog fa-spin"></i></div>');
...
function mostrarDialogoInfoMireserva(cad,cad2,desc,aforo,minimo,disponibles,totales,diasantelacion,minutostope,consumocad,cancelacionminutos,auxiliador){
	...
	modalAddTitle(mymodal,cad);
	if(auxiliador=='NO'){
	$('#reserva_info_modal .modal-body').html('<div style="text-align: center; margin-top: -16px; color: black; font-size: 13px; font-weight: 500;">Con horario '+cad2+'</div><div style="text-align: center;font-size: 11px;color: black;margin-top: 8px;">'+desc+'</div><div style="text-align: center;font-size: 11px;margin-top: 3px;color: black;">Aforo máximo de: '+aforo+' personas</div><div style="text-align: center;font-size: 11px;color: black;">Reservas mínimas para que haya clase: '+minimo+'</div><div style="text-align: center;font-size: 11px;color: black;">Plazas disponibles: '+disponibles+' / Plazas totales: '+totales+'</div><div style="font-size: 11px;color: black;font-weight: 700;text-align: left;margin-top: 8px;">Puedes cancelar la reserva de esta actividad hasta '+cadcancelacionminutos+'</div><div style="font-size: 11px;color: black;text-align: left;margin-top: 8px;">La reserva de esta actividad y tramo horario puede hacerse con un máximo de '+diasantelacion+' días y un mínimo de '+cadminutostope+' de antelación a su comienzo, a menos que el sistema estime oportuno ampliar el plazo.</div><div style="font-size: 11px;color: black;text-align: left;margin-top: 8px;">Actualmente hay plazas disponibles y es posible reservar la actividad.</div><div style="font-size: 11px;color: black;text-align: left;margin-top: 8px;">De mantener la reserva de esta actividad hasta el cierre del periodo de cancelación para la misma, se '+consumocad+'.</div><div style="font-weight:600; font-size: 11px;color: black;text-align: left;margin-top: 8px;">Actualmente hay tiempo para cancelar tu reserva de esta actividad. Dicha acción preservará tu crédito y permitirá que otros atletas puedan ocupar esa plaza.</div><div style="text-align: center;text-transform: uppercase;background-color: #4bacc6;color: white;font-weight: 700;font-size: 12px;margin-top: 10px;cursor: pointer;margin-bottom: 10px;">ir a <span onclick="modal_eula(\'condiciones_uso\');">política de privacidad</span> y <span onclick="modal_eula(\'informacion_legal\');">aviso legal</span></div>');
	}else{
		if(auxiliador=='SI'){
			$('#reserva_info_modal .modal-body').html('<div style="text-align: center; margin-top: -16px; color: black; font-size: 13px; font-weight: 500;">Con horario '+cad2+'</div><div style="text-align: center;font-size: 11px;color: black;margin-top: 8px;">'+desc+'</div><div style="text-align: center;font-size: 11px;margin-top: 3px;color: black;">Aforo máximo de: '+aforo+' personas</div><div style="text-align: center;font-size: 11px;color: black;">Reservas mínimas para que haya clase: '+minimo+'</div><div style="text-align: center;font-size: 11px;color: black;">Plazas disponibles: '+disponibles+' / Plazas totales: '+totales+'</div><div style="font-size: 11px;color: black;font-weight: 700;text-align: left;margin-top: 8px;">Ya no puedes cancelar la reserva de esta actividad.</div><div style="font-size: 11px;color: black;text-align: left;margin-top: 8px;">La reserva de esta actividad y tramo horario puede hacerse con un máximo de '+diasantelacion+' días y un mínimo de '+cadminutostope+' de antelación a su comienzo, a menos que el sistema estime oportuno ampliar el plazo.</div><div style="font-size: 11px;color: black;text-align: left;margin-top: 8px;">Actualmente hay plazas disponibles y es posible reservar la actividad.</div><div style="font-size: 11px;color: black;text-align: left;margin-top: 8px;">De mantener la reserva de esta actividad hasta el cierre del periodo de cancelación para la misma, se '+consumocad+'.</div><div style="font-weight:600; font-size: 11px;color: black;text-align: left;margin-top: 8px;">El plazo para cancelar tu reserva ha terminado.</div><div style="text-align: center;text-transform: uppercase;background-color: #4bacc6;color: white;font-weight: 700;font-size: 12px;margin-top: 10px;cursor: pointer;margin-bottom: 10px;">ir a <span onclick="modal_eula(\'condiciones_uso\');">política de privacidad</span> y <span onclick="modal_eula(\'informacion_legal\');">aviso legal</span></div>');		
		}
	}
} 
...
function mostrarDialogoMiReserva(id,auxiliador){
	...
	$.getJSON(api_url+'schedules/hay_plazas?callback=?', {id:id}, function(data){
		if(data.status=='success'){
			...
			$('#reserva_propia_modal .modal-body').html('  <div style="text-align: center; margin-top: -16px; color: black; font-size: 14px; font-weight: 500;">COACH: '+data.data.coach+'</div><div style="text-align: center; margin-top: 2px; color: black; font-size: 13px; font-weight: 500;">'+data.data.time_start+' - '+data.data.time_end+'</div><div style="text-align: center;color: #07b255;font-weight: 600;font-size: 14px;">DISPONIBLE <i class="fa fa-smile-o"></i></div><div style="text-align: center;margin-top: 9px;font-size: 16px;color: #1e497d;font-weight: 600;">TIENES RESERVADA ESTA ACTIVIDAD</div><div style="text-align: center;font-size: 11px;color: black;margin-top: 8px;">Actualmente dispones de los siguientes créditos:</div><div style="text-align: center;font-size: 11px;margin-top: 3px;color: black;">WOD: '+data.data.credit_wod+' de un total de '+creditito_wod+'</div><div style="text-align: center;font-size: 11px; color: black;">OPEN BOX: '+data.data.credit_box+' de un total de '+creditito_box+'</div><div style="text-align: center;font-size: 11px;color: black;">BONO PREPAGO: '+data.data.credit_bono+' créditos</div><div style="font-size: 11px;color: black;text-align: center;margin-top: 8px;">Mantener la reserva de actividad hasta su finalización, consumirá uno de tus créditos.</div><div style="overflow: auto;margin-top: 8px;"><div style="clear: both;color: white;background-color: #e36c0a;font-weight: 700;font-size: 20px;text-align: center;padding-right: 6px;margin-left: auto;cursor: pointer;margin-right: auto;" onclick="cerrarDialogoMiReserva();">MANTENER MI RESERVA</div><div style="font-size: 20px;text-align: center;clear: both;font-weight: 700;color: white;background-color: #ff0000;cursor: pointer;margin-left: auto;margin-right: auto;margin-top: 15px;" onclick="deleteMiReserva('+id+');">CANCELAR MI RESERVA</div></div><div style="font-size: 11px; color: black; padding-left: 13px; margin-top: 10px;">Actividad actualmente también reservada por:</div><ol style="display: block;padding-left: 27px;margin-top: 6px;">'+reservadores+'</ol><div style="text-align: center;"><i onclick="mostrarDialogoInfoMireserva(\''+cad+'\',\''+data.data.time_start+' - '+data.data.time_end+'\',\''+data.data.activity_desc+'\','+data.data.aforo+','+data.data.minimo+','+disponibles+','+data.data.aforo+','+data.data.days_pre+','+data.data.minutes_pre+',\''+consumocad+'\','+data.data.minutes_post+',\''+auxiliador+'\');" class="fa fa-info-circle" style="cursor: pointer; font-size: 26px; color: black;"></i></div><div onclick="mostrarDialogoInfoMireserva(\''+cad+'\',\''+data.data.time_start+' - '+data.data.time_end+'\',\''+data.data.activity_desc+'\','+data.data.aforo+','+data.data.minimo+','+disponibles+','+data.data.aforo+','+data.data.days_pre+','+data.data.minutes_pre+',\''+consumocad+'\','+data.data.minutes_post+',\''+auxiliador+'\');" style="text-align: center;text-transform: uppercase;background-color: #8064a2;color: white;font-weight: 700;font-size: 12px;cursor: pointer;margin-bottom: 10px;">Ver condiciones e información adicional</div>');
		}else launch_alert('<i class="fa fa-frown-o"></i> '+data.response,'warning');
	});
}
...
function addReservaCliente(id){
	...
	modalAddBody(mymodal,'<div class="waiting"><i class="fa fa-cog fa-spin"></i></div>');
	mymodal.modal('show');
	$('#reserva_cliente_modal .close').hide();
	$('#reserva_cliente_modal .modal-dialog').attr('style','width: 330px');
	$('#reserva_cliente_modal .modal-content').attr('border-radius','12px !important');
	$('#reserva_cliente_modal .modal-footer').hide();
	$('#reserva_cliente_modal .modal-title').attr('style','text-transform: capitalize; color: black; text-align: center; font-weight: 600; font-size: 15px; margin-top: 8px;')
	$.getJSON(api_url+'schedules/hay_plazas?callback=?', {id:id}, function(data){
		if(data.status=='success'){
			var cad = data.data.activity + ' ';
			if(data.data.dia_semana==0){
			...
			if(data.data.credit_box_total==0){
				creditito_box = data.data.credit_box;
						...
						$('#reserva_cliente_modal .modal-body').html('  <div style="text-align: center; margin-top: -16px; color: black; font-size: 14px; font-weight: 500;">COACH: '+data.data.coach+'</div><div style="text-align: center; margin-top: 2px; color: black; font-size: 13px; font-weight: 500;">'+data.data.time_start+' - '+data.data.time_end+'</div><div style="text-align: center;color: #07b255;font-weight: 600;font-size: 14px;">DISPONIBLE <i class="fa fa-smile-o"></i></div><div style="text-align: center;margin-top: 9px;font-size: 16px;color: #1e497d;font-weight: 600;">VAS A EFECTUAR UNA RESERVA PARA ESTA ACTIVIDAD</div><div style="text-align: center;font-size: 11px;color: black;margin-top: 8px;">Actualmente dispones de los siguientes créditos:</div><div style="text-align: center;font-size: 11px;margin-top: 3px;color: black;">WOD: '+data.data.credit_wod+' de un total de '+creditito_wod+'</div><div style="text-align: center;font-size: 11px; color: black;">OPEN BOX: '+data.data.credit_box+' de un total de '+creditito_box+'</div><div style="text-align: center;font-size: 11px;color: black;">BONO PREPAGO: '+data.data.credit_bono+' créditos</div><div style="font-size: 11px;color: black;text-align: center;margin-top: 8px;">Mantener la reserva de actividad hasta su finalización, consumirá uno de tus créditos.</div><div style="font-size: 11px;color: black;text-align: center;margin-top: 8px;"><input type="checkbox" id="inputsuperimportante"> Acepto el <span onclick="modal_eula(\'informacion_legal\');" style="text-decoration: underline; cursor: pointer;">aviso legal</span> y la <span onclick="modal_eula(\'condiciones_uso\');" style="text-decoration: underline; cursor: pointer;">política de privacidad</span>.</div><div style="overflow: auto;margin-top: 8px;"><div style="float: left;padding-left: 6px;color: white;background-color: #92d050;font-weight: 700;font-size: 20px;text-align: center;padding-right: 6px;margin-left: 26px;cursor: pointer;" onclick="addReservaClienteFinal('+id+');">ACEPTAR</div><div style="float: left;font-size: 20px;padding-left: 2px;font-weight: 700;color: white;background-color: #ff0000;padding-right: 2px;cursor: pointer;margin-left: 16px;" onclick="cerrarDialogoReserva();">CANCELAR</div></div><div style="font-size: 11px; color: black; padding-left: 13px; margin-top: 10px;">Actividad actualmente también reservada por:</div><ol style="display: block;padding-left: 27px;margin-top: 6px;">'+reservadores+'</ol><div style="text-align: center;"><i onclick="mostrarDialogoInfo(\''+cad+'\',\''+data.data.time_start+' - '+data.data.time_end+'\',\''+data.data.activity_desc+'\','+data.data.aforo+','+data.data.minimo+','+disponibles+','+data.data.aforo+','+data.data.days_pre+','+data.data.minutes_pre+',\''+consumocad+'\','+data.data.minutes_post+');" class="fa fa-info-circle" style="cursor: pointer; font-size: 26px; color: black;"></i></div><div onclick="mostrarDialogoInfo(\''+cad+'\',\''+data.data.time_start+' - '+data.data.time_end+'\',\''+data.data.activity_desc+'\','+data.data.aforo+','+data.data.minimo+','+disponibles+','+data.data.aforo+','+data.data.days_pre+','+data.data.minutes_pre+',\''+consumocad+'\','+data.data.minutes_post+');" style="text-align: center;text-transform: uppercase;background-color: #8064a2;color: white;font-weight: 700;font-size: 12px;cursor: pointer;margin-bottom: 10px;">Ver condiciones e información adicional</div>');
					}else{
						var consumocad = '';
						if(data.data.consume_wod>1){
							consumocad = 'consumirán '+data.data.consume_wod+' créditos';
			...
			if(data.data.credit_box_total==0){
				creditito_box = data.data.credit_box;
						...
						$('#reserva_cliente_modal .modal-body').html('  <div style="text-align: center; margin-top: -16px; color: black; font-size: 14px; font-weight: 500;">COACH: '+data.data.coach+'</div><div style="text-align: center; margin-top: 2px; color: black; font-size: 13px; font-weight: 500;">'+data.data.time_start+' - '+data.data.time_end+'</div><div style="text-align: center;color: #07b255;font-weight: 600;font-size: 14px;">DISPONIBLE <i class="fa fa-smile-o"></i></div><div style="text-align: center;margin-top: 9px;font-size: 16px;color: #ff0000;font-weight: 600;">NO DISPONES DE CRÉDITOS SUFICIENTES PARA REALIZAR UNA RESERVA</div><div style="text-align: center;font-size: 11px;color: black;margin-top: 8px;">Actualmente dispones de los siguientes créditos:</div><div style="text-align: center;font-size: 11px;margin-top: 3px;color: black;">WOD: '+data.data.credit_wod+' de un total de '+creditito_wod+'</div><div style="text-align: center;font-size: 11px; color: black;">OPEN BOX: '+data.data.credit_box+' de un total de '+creditito_box+'</div><div style="text-align: center;font-size: 11px;color: black;">BONO PREPAGO: '+data.data.credit_bono+' créditos</div><div style="overflow: auto;margin-top: 8px;"><div style="font-size: 20px;padding-left: 2px;font-weight: 700;color: white;background-color: #ff0000;padding-right: 2px;cursor: pointer;margin-left: auto;margin-right: auto;text-align: center;width: 120px;" onclick="cerrarDialogoReserva();">CANCELAR</div></div><div style="font-size: 11px; color: black; padding-left: 13px; margin-top: 10px;">Actividad actualmente también reservada por:</div><ol style="display: block;padding-left: 27px;margin-top: 6px;">'+reservadores+'</ol><div style="text-align: center;"><i onclick="mostrarDialogoInfo(\''+cad+'\',\''+data.data.time_start+' - '+data.data.time_end+'\',\''+data.data.activity_desc+'\','+data.data.aforo+','+data.data.minimo+','+disponibles+','+data.data.aforo+','+data.data.days_pre+','+data.data.minutes_pre+',\''+consumocad+'\','+data.data.minutes_post+');" class="fa fa-info-circle" style="cursor: pointer; font-size: 26px; color: black;"></i></div><div onclick="mostrarDialogoInfo(\''+cad+'\',\''+data.data.time_start+' - '+data.data.time_end+'\',\''+data.data.activity_desc+'\','+data.data.aforo+','+data.data.minimo+','+disponibles+','+data.data.aforo+','+data.data.days_pre+','+data.data.minutes_pre+',\''+consumocad+'\','+data.data.minutes_post+');" style="text-align: center;text-transform: uppercase;background-color: #8064a2;color: white;font-weight: 700;font-size: 12px;cursor: pointer;margin-bottom: 10px;">Ver condiciones e información adicional</div>');
					}
				}else{
			...
			if(data.data.credit_box_total==0){
				creditito_box = data.data.credit_box;
						...
						$('#reserva_cliente_modal .modal-body').html('  <div style="text-align: center; margin-top: -16px; color: black; font-size: 14px; font-weight: 500;">COACH: '+data.data.coach+'</div><div style="text-align: center; margin-top: 2px; color: black; font-size: 13px; font-weight: 500;">'+data.data.time_start+' - '+data.data.time_end+'</div><div style="text-align: center;color: #ff0000;font-weight: 600;font-size: 14px;">NO DISPONIBLE <i class="fa fa-frown-o"></i></div><div style="text-align: center;margin-top: 9px;font-size: 19px;color: black;margin-bottom: 8px;font-weight: 600;">AFORO COMPLETO</div><div style="text-align: center;margin-top: 9px;font-size: 16px;color: #ff0000;font-weight: 600;">NO DISPONES DE CRÉDITOS SUFICIENTES PARA REALIZAR UNA RESERVA</div><div style="text-align: center;font-size: 11px;color: black;margin-top: 8px;">Actualmente dispones de los siguientes créditos:</div><div style="text-align: center;font-size: 11px;margin-top: 3px;color: black;">WOD: '+data.data.credit_wod+' de un total de '+creditito_wod+'</div><div style="text-align: center;font-size: 11px; color: black;">OPEN BOX: '+data.data.credit_box+' de un total de '+creditito_box+'</div><div style="text-align: center;font-size: 11px;color: black;">BONO PREPAGO: '+data.data.credit_bono+' créditos</div><div style="overflow: auto;margin-top: 8px;"><div style="font-size: 20px;padding-left: 2px;font-weight: 700;color: white;background-color: #ff0000;padding-right: 2px;cursor: pointer;margin-left: auto;margin-right: auto;text-align: center;width: 120px;" onclick="cerrarDialogoReserva();">CANCELAR</div></div><div style="font-size: 11px; color: black; padding-left: 13px; margin-top: 10px;">Actividad actualmente también reservada por:</div><ol style="display: block;padding-left: 27px;margin-top: 6px;">'+reservadores+'</ol><div style="text-align: center;"><i onclick="mostrarDialogoInfoCompleta(\''+cad+'\',\''+data.data.time_start+' - '+data.data.time_end+'\',\''+data.data.activity_desc+'\','+data.data.aforo+','+data.data.minimo+','+disponibles+','+data.data.aforo+','+data.data.days_pre+','+data.data.minutes_pre+',\''+consumocad+'\','+data.data.minutes_post+');" class="fa fa-info-circle" style="cursor: pointer; font-size: 26px; color: black;"></i></div><div onclick="mostrarDialogoInfoCompleta(\''+cad+'\',\''+data.data.time_start+' - '+data.data.time_end+'\',\''+data.data.activity_desc+'\','+data.data.aforo+','+data.data.minimo+','+disponibles+','+data.data.aforo+','+data.data.days_pre+','+data.data.minutes_pre+',\''+consumocad+'\','+data.data.minutes_post+');" style="text-align: center;text-transform: uppercase;background-color: #8064a2;color: white;font-weight: 700;font-size: 12px;cursor: pointer;margin-bottom: 10px;">Ver condiciones e información adicional</div>');
...
function addReservaClienteFinal(id){
	...
	if(confirmacion){
		...
		$.getJSON(api_url+'schedules/hay_plazas?callback=?', {id:id}, function(data){
			if(data.status=='success'){
				var disponibles = data.data.disponibles;
				var sucesiva = data.data.sucesiva;
				var disponibles_cola = data.data.disponibles_cola;
				var minutos = data.data.minutos;
										...
										if(data.status=='success'){
											...
											if(parseInt(minutos)>100){
													...
													if(horas>1){
														...
														var horajs = parseInt(horacomienzo.split(':')[0]);
				...
				launch_alert('Lo sentimos, ya has reservado plaza en una actividad próxima a esta franja horaria.','warning');
				$('.waiting').hide();
				$('#reservas-tabla').show();
			}
			}
