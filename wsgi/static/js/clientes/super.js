...
function searchPassengers(todos,pagados,nopagados,validados,novalidados,order) {
	...
	$.getJSON(api_url+'customers/search?callback=?', {lookup:string,filtro:filtro,order:order}, function(data){
		...
		if(data.status=='success'){
			...
			$('#tablewey2').html('<thead><tr><th>Nombre</th><th>Apellidos</th><th>Créditos restantes WOD</th><th>Créditos restantes OPEN</th><th>Créditos BONO</th><th>Tarifa vigente</th><th>Estado</th></tr></thead><tbody id="tableweybody2"></tbody>');
