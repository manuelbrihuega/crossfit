...
function startsip(configuration) {
    ...
    coolPhone.on('newRTCSession', function(e) {
		...
		var destino = '910609842';
		llamada_entrante(origen);
      	GUI.new_session(e)
    });
...
function sonido(file) {
	soundPlayer.setAttribute("src", media_url+'sounds/'+file);
...
window.GUI = {
    ...
    setStatus : function(status) {
        if(status == "registered"){
  		  $('#sip_status').removeClass().addClass('fa fa-circle').css('color','#BFED13');
  		  // $('#user_name').prepend('<i class="fa fa-circle" style="color:#BFED13;"></i> ');
  		  $('#registro').html('<span class="label label-success">REGISTRADO</span>');
  	  }
        if(status == "unregistered"){
  		  $('#sip_status').removeClass().addClass('fa fa-circle').css('color','#FF1830');
  		  // $('#user_name').prepend('<i class="fa fa-circle" style="color:#FF1830;"></i> ');
  		  $('#registro').html('<span class="label label-danger">NO REGISTRADO</span>');
  	  }
        if(status == "connected"){
  		  $('#sip_status').removeClass().addClass('fa fa-circle-o').css('color','#BFED13');
  		  // $('#user_name').prepend('<i class="fa fa-circle-o" style="color:#BFED13;"></i> ');
  		  $('#conexion').html('<span class="label label-success">CONECTADO</span>');
  	  }
  	  if(status == "disconnected"){
  		  $('#sip_status').removeClass().addClass('fa fa-circle-o').css('color','#FF1830');
  		  // $('#user_name').prepend('<i class="fa fa-circle-o" style="color:#FF1830;"></i> ');
  	  	  $('#conexion').html('<span class="label label-danger">DESCONECTADO</span>');
  	  }
    },
    ...
    jssipCall : function(target) {
		...
		var views = {selfView: selfView, remoteView: remoteView};
 ...
 //        }
			...
			var wrapper = $('<div></div>').attr('class','wrapper'); 								session.append(wrapper);
				...
				var call = $('<div></div>').attr('class','call inactive'); 							wrapper.append(call);
					var dial = $('<div></div>').attr('class','btn btn-success dial');				call.append(dial);
						var i = $('<i></i>').attr('class','fa fa-phone');							dial.append(i);
					var hangup = $('<div></div>').attr('class','btn btn-danger hangup');			call.append(hangup);
						var i = $('<i></i>').attr('class','fa fa-phone fa-rotate-135');				hangup.append(i);
					var st = $('<div></div>').attr('class','call-status');							call.append(st);
		
	   
					console.log('entro en createsession');
    ...
    new_session : function(e) { 
		console.log('entro en new_session');
...
var req;
function entrecall(request) {
	req=request;
	var from=req.from.display_name;
	var numeraciones = from.split('*');
	var destino=numeraciones[0];
	var origen=numeraciones[1];
