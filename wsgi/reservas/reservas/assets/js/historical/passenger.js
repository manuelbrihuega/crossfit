...
function get_content() {
	...
	$.post('partials/historical_passenger', function(template, textStatus, xhr) {
		$('#main').html(template);
		$.getJSON(api_url+'passengers/count_by_months?callback=?', '', function(data){
...
function addPanel(group) {
	...
	var panel=$('<div></div>').attr('class','panel panel-default'); $('#historical_accordion').append(panel);
		...
		var collapse=$('<div></div>').attr({'class':'panel-collapse collapse','id':'group_'+group.year+'_'+group.month}); panel.append(collapse);
			...
			$.getJSON(api_url+'passengers/journeys_by_month?callback=?', {month:group.month, year:group.year, offset:local_offset}, function(data){
				if(data.status=='success'){
