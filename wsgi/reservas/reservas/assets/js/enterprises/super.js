function get_content() {
    ...
    $.when(
		...
		$.post('partials/enterprises_super', function(template, textStatus, xhr) {
			$('#main').html(template);
...
function loadPending() {
	...
	wrapper.empty();
	$.getJSON(api_url+'enterprises/list_pending?callback=?', function(data){
		if(data.status=='success'){
			if(data.data.length>0){
				$.each(data.data, function(index, empresa) {
					draw_enterprise_sm(empresa, wrapper, '');
...
function getEnterprisesList() {
	...
	$.getJSON(api_url+'enterprises/list_all?callback=?', '', function(data){
		if(data.status=='success'){
