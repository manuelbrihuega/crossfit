...
def add_contact_method(telegram,name,nick,phone,msg):
    ...
    index = telegram.expect(['0m', 'successfully', pexpect.EOF, pexpect.TIMEOUT],timeout=5)
    if index == 0 or index == 1:
...
def send_msg_method(telegram, nick, msg):
    ...
    index = telegram.expect(['0m', 'print_message', 'Bad', pexpect.EOF, pexpect.TIMEOUT],timeout=5)
    if index == 0 or index == 1:
        print 'SENT'
    elif index == 2:
        print 'BAD_USER_SEND'
