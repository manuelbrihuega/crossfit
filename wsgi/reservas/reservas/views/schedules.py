...
def delete(request):
    ...
    try:
        ...
        for res in reservations:
            ...
            add_task(datetime.utcnow(),'send_email_cancel_reservation_task(auth_id="'+str(res.auth.id)+'",res_id="'+str(res.id)+'")')
...
def delete_reservation(request):
    ...
    try:
        ...
        if reservation.queue:
            ...
            add_task(datetime.utcnow(),'send_email_cancel_reservation_task(auth_id="'+str(user.auth.id)+'",res_id="'+str(reservation.id)+'")')
            add_task(datetime.utcnow(),'send_telegram_task(name="'+name+'",nick="'+nick+'",phone="'+phone+'",msg="'+message+'")')
            reservation.delete()
            reservations=Reservations.objects.filter(Q(schedule_time__id=schedule_time_id) & Q(queue=True))
            for res in reservations:
