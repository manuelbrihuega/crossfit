                ...
                if cadmeses[fechaprincipal.month-1]=='1':
                    if cadsemana[fechaprincipal.weekday()]=='1':
                        ...
                        if timbre:
                            ...
                            definitiva = semidefinitiva.astimezone(pytz.utc)
                            fechita = datetime(definitiva.year, definitiva.month, definitiva.day, definitiva.hour, definitiva.minute, 0)
                            add_task(fechita,'revise_schedule_task(idschedule="'+str(schedule_timeaux.id)+'")')
            ...
            if validate_parameter(request.GET,'id'):
                try:
                    ...
                    if yearactual == yearactividad and monthactividad == (monthactual + 1):
                        credit_wod = customer.rate.credit_wod 
                        credit_box = customer.rate.credit_box
                    else:
...
def add_reservation(request):
    ...
    try:
        ...
        if disponibles > 0:
            ...
            definitiva = semidefinitiva.astimezone(pytz.utc)
            fechita = datetime(definitiva.year, definitiva.month, definitiva.day, definitiva.hour, definitiva.minute, 0)
            add_task(fechita,'revise_reservation_task(idreservation="'+str(reservation.id)+'")')
            name = 'User_Id'+str(auth.id)
            nick = 'User_Id'+str(auth.id)
            phone = '+34'+str(auth.phone)
            conf=Configuration.objects.get(id=1)
...
def rm_concrete(request):
    try:
        ...
        while contadortramos < num:
            ...
            for sch in schedule:
                cad = cad + str(sch.id)
                if sch.date.year == int(fechazocad[0]) and sch.date.month == int(fechazocad[1]) and sch.date.day == int(fechazocad[2]):
                    cad = cad + 'ole'
