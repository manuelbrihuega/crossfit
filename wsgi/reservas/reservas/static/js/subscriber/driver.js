...
function addPanel(group) {
	...
	var panel=$('<div></div>').attr('class','panel panel-default'); $('#subscriber_accordion').append(panel);
		...
		var collapse=$('<div></div>').attr({'class':'panel-collapse collapse in','id':'group_'+group.year+'_'+group.month}); panel.append(collapse);
			...
			$.getJSON(api_url+'drivers/journeys_corporate_by_month?callback=?', {month:group.month, year:group.year, offset:local_offset}, function(data){
							...
							var table=$('<table></table>').attr('class','table tableSorter'); table_wrapper.append(table);
								var thead=$('<thead></thead>'); table.append(thead);
									var tr=$('<tr></tr>'); thead.append(tr);
										...
										var th=$('<th></th>').text('Conductor'); tr.append(th);
...
function selectMonth(month){
	...
	$.getJSON(api_url+'drivers/count_corporate_by_months?callback=?', '', function(data){
		if(data.status=='success'){
			if(data.data.length>0){
...
function loadFirstGraphical(month,year){
    ...
    $.getJSON(api_url+'drivers/get?callback=?', function(data){
		if(data.status=='success'){
			var currency = data.data.driver_profile.currency;
			$.getJSON(api_url+'drivers/list_income_corporate_by_month?callback=?', {	
																month:month_view+1,
...
function loadGraphicalJourneysYear(){
	...
	$("#cvs").attr({'width':width});
	$("#cvs").attr({'height':height});
	$.getJSON(api_url+'drivers/list_journeys_corporate_by_year?callback=?', {	
																year:year_view}, function(data2){
...
function loadGraphicalIncomesYear(){
    ...
    $.getJSON(api_url+'drivers/get?callback=?', function(data){
		if(data.status=='success'){
			var currency = data.data.driver_profile.currency;
			$.getJSON(api_url+'drivers/list_income_corporate_by_year?callback=?', {	
    															year:year_view}, function(data2){
...
function loadGraphicalJourneysMonth(){
	...
	$("#cvs").attr({'width':width});
	$("#cvs").attr({'height':height});
	$.getJSON(api_url+'drivers/list_journeys_corporate_by_month?callback=?', {	
    															month:month_view+1,
																year:year_view}, function(data2){
